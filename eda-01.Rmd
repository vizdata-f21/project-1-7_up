...
income_mean <- income_mean %>%
  ...
  filter(race == "All" | race == "Asian" | race == "Black" | 
           race == "Hispanic" | race == "White")
...
income_mean$income_percentile <- factor(income_mean$income_percentile, 
                                        levels = c("1st - 5th", "1st - 20th", "21st - 40th",
...
ggplot(data = income_mean,
  ...
  labs(title = "Income Quintiles: Time Series Across Different Races",
       subtitle = "7 Up: Blossom Mojekwu, Kartik Chamarti, Margaret Reed, Phillip Harmadi",
       x = "Year", y = "Household Income (2019 Dollars)", color = "Percentile",
...
income_mean_wider <- pivot_wider(data = income_mean, 
...
income_mean_wider$race <- factor(income_mean_wider$race, 
                                 levels = c("White", "Black", "Hispanic", "Asian"))

income_mean_wider

# income_mean_wider$lowest_standardized <- as.double(income_mean_wider$lowest_standardized)
# income_mean_wider$second_standardized <- as.double(income_mean_wider$second_standardized)
# income_mean_wider$middle_standardized <- as.double(income_mean_wider$middle_standardized)
# income_mean_wider$fourth_standardized <- as.double(income_mean_wider$fourth_standardized)
# income_mean_wider$highest_standardized <- as.double(income_mean_wider$highest_standardized)
# income_mean_wider$top_5_pct_standardized <- as.double(income_mean_wider$top_5_pct_standardized)
...
income_mean_final <- income_mean_wider %>% 
  pivot_longer(data = income_mean_wider, 
               cols = !year:race,
...
ggplot(data = income_mean_final,
  ...
  geom_line(size = 0.25, alpha = 0.75) + 
  facet_wrap(. ~ race, nrow = 2) +
  labs(title = "Income Gap: How much do top households earn compared to bottom households for each race?",
       subtitle = "7 Up: Blossom Mojekwu, Kartik Chamarti, Margaret Reed, Phillip Harmadi",
...
income_mean_final %>%
  ...
  ggplot(aes(fill = income_quintile, y = income_dollars_standardized, x = race)) + 
