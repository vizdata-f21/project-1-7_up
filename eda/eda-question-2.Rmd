...
library(readr)
income_mean_1 <- read_csv("../data/income_mean.csv")
...
```{r}
income_mean_final %>%
  ggplot(aes(fill = income_quintile,
             x = 1,
             y = income_dollars_standardized)) + 
  geom_col(width = 1, position = "fill") +
  coord_polar(theta = "y") +
  facet_wrap(. ~ race, nrow = 2) +
  labs(title = "Proportion of income quintiles across racial groups")
...
#the following dataframe that I created shows the total values for each box based on race and income quintile. 
# finding a way to merge both dataframes should allow it so that the geom_text label
# can access the data.
# link for help: https://stackoverflow.com/questions/6644997/showing-data-values-on-stacked-bar-chart-in-ggplot2

income_mean_use <- income_mean_final %>%
  mutate(income_quintile = fct_reorder(.f = income_quintile, .x = income_dollars_standardized, 
                                     .fun = mean, na.rm = FALSE, .desc = TRUE)) %>%
  filter(year == 2019) %>%
  filter(income_quintile != "top_5_pct_standardized")
income_mean_sum <- income_mean_final %>%
  select(race,income_percentile, income_dollars_standardized) %>%
  group_by(race,income_percentile) %>%
  summarise(total_value = sum(income_dollars_standardized))
#rename(income_mean_sum, income_percentile_sum = income_percentile)
#rename(income_mean_sum, race_sum = race)

income_mean_use <- bind_cols(income_mean_sum, income_mean_use)
ggplot(check, aes(fill = income_percentile, 
             x = income_dollars_standardized, 
  ...
  labs(
    ...
    caption = "Text: Household income ratio as compared to the bottom quintile (81st-100th)."
    ) +
  scale_colour_viridis_d() +
  ...
  #geom_text(aes(label = round(total_value,1)),size = 3, position = position_stack(vjust = 0.5))
...
income_mean %>%
  ...
  scale_y_continuous(labels = scales::label_number(big.mark = ",", 
                                                   ...
                                                   suffix = "k",
                                                   prefix = "$")) +
  scale_color_brewer(type = "seq", palette = "Spectral", direction = -1) +
  labs(
    title = "Income Gap: How does income distribution each race?",
    subtitle = "7 Up: Blossom Mojekwu, Kartik Chamarti, Margaret Reed, Phillip Harmadi",
